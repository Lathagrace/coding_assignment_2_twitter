...
const getPeopleIdsOfUser = async (username) => {
  const getPeopleQuery = `
       SELECT following_user_id FROM follower
       INNER JOIN user ON user.user_id = follower.follower_user_id
       WHERE user.username = '${username}';`;
  const followingPeople = await db.all(getPeopleQuery);
...
app.get("/user/tweets/feed/", authentication, async (request, response) => {
  ...
  const followingPeopleIds = await getPeopleIdsOfUser(username);
...
app.get(
  ...
  async (request, response) => {
    const { tweetId } = request.params;
    const getLikesQuery = `SELECT username FROM user
